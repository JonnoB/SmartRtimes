...
d2 <- read.csv("HalfHourlyDataSource.csv", check.names = FALSE) %>% mutate(TimeZoneCol = ifelse(`Data provider` == "Logica",
                                              "GMT",
                                              "Europe/London")) %>%
...
#This needs to be simplified so that the joining of the timezone is not part of the function
 ...
 x %>% 
    left_join(., mutate(Convdf, `Location ID` = as.character(`Location ID`)), by= "Location ID") %>%
