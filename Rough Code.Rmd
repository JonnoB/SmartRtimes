...
filt <- function(df, Date.Time, TimeZone, Start=16, End=21, segments=1000){
...
f <- function(x, pos) filt(x, Date.Time = "Date and Time of capture", "GMT")

test <- read_csv_chunked("ScanTest.csv", DataFrameCallback$new(f), chunk_size = 5)



total <- scan("TrialMonitoringDataHH.csv", what=list(NULL),sep='\n', blank.lines.skip = F)

total <- countLines("TrialMonitoringDataHH.csv")

test <- read_lines("TrialMonitoringDataHH.csv", 1000) 

test2 <- test %>% select_(-2, -3)

test3 <- test2 %>% setNames(c("ID", "Klang", "Value")) %>% 
  FilterTime(., Date.Time = "Klang", "GMT", segments = 100)
